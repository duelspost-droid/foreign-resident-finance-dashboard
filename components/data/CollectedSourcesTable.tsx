  ...
  const rows = useMemo(() => {
    ...
    return sources
      .filter((s) => {
        ...
        const surfaced = Boolean(SURFACED[s.id]);
        if (link === "surfaced" && !surfaced) return false;
        if (link === "unsurfaced" && surfaced) return false;
        if (needle) {
          const hay = `${s.title} ${s.provider} ${s.id} ${targetLabel(s.targetTable)}`.toLowerCase();
      ...
      .sort((a, b) => {
        ...
        const aw = (SURFACED[a.id] ? 0 : 1) + (a.status === "downloaded" ? 0 : 2);
        const bw = (SURFACED[b.id] ? 0 : 1) + (b.status === "downloaded" ? 0 : 2);
        return bw - aw || (b.rowCount ?? 0) - (a.rowCount ?? 0);
      });
  }, [sources, q, status, link, approvedOnly]);

  const unsurfaced = sources.filter((s) => !SURFACED[s.id]).length;
  const approved = sources.filter((s) => s.id.startsWith("approved_")).length;

  return (
    <section className="surface">
      <div className="surface-header flex-col items-start gap-3 sm:flex-row sm:items-center">
        <div>
          <h3 className="surface-title">전체 수집 원본 ({sources.length})</h3>
            ...
            {rows.map((s) => {
              ...
              return (
                <tr key={s.id} className="border-b border-slate-100 align-top last:border-0">
                  ...
                  <td className="px-4 py-2.5 text-xs text-slate-600">{targetLabel(s.targetTable)}</td>
